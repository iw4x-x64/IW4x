...
namespace iw4x
{
  // @@ This may not be the best idea to expose the entire utility namespace
  // here. Perhaps we should only expose specific utility objects (e.g.,
  // utility::obj). But for now, we export everything and treat the import as a
  // single, monolithic, interwoven symbol graph. On the upside, this allows for
  // convenient usage like:
  //
  //   #include <import.hxx>
  //   #include <export.hxx>
  //
  ...
  inline console_show_t console_show = reinterpret_cast<console_show_t> (0x1402AB600);

  using  console_output_t = void (*) (const char*);
  inline console_output_t console_output = reinterpret_cast<console_output_t> (0x1402AAE00);
  ...
  inline cmd_execute_single_command_t cmd_execute_single_command = reinterpret_cast<cmd_execute_single_command_t> (0x1401ECD60);
