  ...
  extern "C"
  {
    ...
    DllMain (HINSTANCE, DWORD r, LPVOID)
    {
      ...
      uintptr_t s (reinterpret_cast<uintptr_t> (+[] ()
      {
        ...
        // Patch online connection phase (likely GDK/Demonware) to always be
        // ready. From IDA, we guess the original routine maps an internal state
        // machine to:
        //
        // 0 - offline/disabled
        // 1 - in-progress
        // 2 - fully connected/ready
        //
        memwrite (0x14012F720, "\xB8\x02\x00\x00\x00\xC3", 6);
