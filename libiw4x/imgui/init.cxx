  ...
  namespace imgui
  {
    namespace
    {
      ...
      render_frame (IDirect3DDevice9* device)
      {
        ...
        // Rendering closes the loop by handing ImGui's draw data back to
        // DirectX. At this point the integration is complete for the frame:
        // ImGui advances in lockstep with the renderer without maintaining
        // separate lifecycles.
        //
        ImGui::Render ();
      ...
      wnd_proc (HWND h, UINT msg, WPARAM wp, LPARAM lp)
      {
        ...
        if (original_wnd_proc != nullptr)
          return CallWindowProcW (original_wnd_proc, h, msg, wp, lp);

        // If no original procedure was stored, fall back to Windows default
        // handler.
        //
        // @@: Likely similar or equivalent to above, if so
        //     we can probably ignore the original proc.
        //
        return DefWindowProcW (h, msg, wp, lp);
      }

      void
      on_frame (IDirect3DDevice9* d)
      {
        // Initialize ImGui lazily on the first frame. We avoid frontloading
        // this work during init since we need a valid device anyway, and there
        // is no point in doing it twice if the first frame never arrives.
        //
        call_once (imgui_init, [d] ()
        {
          ...
          // Direct3D does not hand this out directly. That is, the swap chain
          ...
          // With the handle resolved we can commit to an ImGui context. Inputs
          // are enabled up front to avoid layering configuration later. Note
          // that we rely on ImGui's defaults for most behavior.
          //
          ImGui::CreateContext ();
          ...
          if (original_wnd_proc == nullptr)
            throw runtime_error ("unable to hook window procedure");
        });
    ...
    init ()
    {
      // Register our frame callback with the renderer.
      //
