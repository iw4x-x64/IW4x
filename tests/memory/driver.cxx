...
main ()
{
  ...
  //
  {
    void* result (memwrite (nullptr, 0x42, 0));
  ...
  //
  {
    ...
    void* result (memwrite (buffer.data (), 0x42, buffer.size ()));
  ...
  //
  {
    ...
    memwrite (buffer.data (), 0xFF, buffer.size ());
    for (size_t i (0); i != buffer.size (); ++i)
      assert (buffer [i] == 0xFF);

    memwrite (buffer.data (), 0x00, buffer.size ());
    for (size_t i (0); i != buffer.size (); ++i)
      assert (buffer [i] == 0x00);

    memwrite (buffer.data (), 0xAA, buffer.size ());
    for (size_t i (0); i != buffer.size (); ++i)
  ...
  //
  {
    ...
    memwrite (buffer.data (), 0x90, buffer.size ());
  ...
  //
  {
    ...
    memwrite (buffer.data (), 0x77, buffer.size ());
  ...
  {
    uintptr_t addr (0);
    void* result (memwrite (addr, 0x42, 0));
    assert (result == nullptr);
  }
  ...
  //
  {
    ...
    void* result (memwrite (addr, 0x33, buffer.size ()));
  ...
  //
  {
    ...
    memwrite (addr, 0x90, buffer.size ());
  ...
  //
  {
    ...
    // Set middle 50 bytes
    //
    memwrite (buffer.data () + 25, 0x11, 50);
  ...
  //
  {
    ...
    memwrite (buffer.data (), 0x11, buffer.size ());
    for (size_t i (0); i != buffer.size (); ++i)
      assert (buffer [i] == 0x11);

    memwrite (buffer.data (), 0x22, buffer.size ());
    for (size_t i (0); i != buffer.size (); ++i)
      assert (buffer [i] == 0x22);

    memwrite (buffer.data (), 0x33, buffer.size ());
    for (size_t i (0); i != buffer.size (); ++i)
      assert (buffer [i] == 0x33);
  }
  ...
  //
  {
    ...
    memwrite (buffer.data (), 0xAA, 60);
    memwrite (buffer.data () + 40, 0xBB, 60);
  ...
  //
  {
    ...
    void* result (memwrite (ptr, 0x99, buffer.size ()));
